...
@Component({
  selector: 'app-users',
  standalone: true,
  imports: [NgForOf, NgIf, RouterOutlet],
  template: `
    <div *ngFor="let user of users; index as i">
      {{ user.email }}
      <ng-container *ngIf="i > 0 && isAdminForEntrySignedIn(i)">
        <button (click)="approveUser(user)" [disabled]="user.isAdmin">
          Approve{{ user.isAdmin ? 'd' : '' }}
        </button>
...
export class UsersComponent {
  ...
  async approveUser(user: any) {
    ...
    const { data, error } = await this.supabaseService.inviteByEmail(
      user.email
    );
  ...
  async makeUserAdmin(id: number) {
    const { data, error } = await this.supabaseService.updateUser(
      { isAdmin: true },
      id
    );
    if (!error) {
